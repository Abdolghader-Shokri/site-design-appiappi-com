...
- a `WP_Customize_Image_Control` upload → `--page-header-bg-{key}`, falling back to that page's default SVG;
...
**Single-design page — its own background, separate from the archive (revised 2026-09-06):** `single-appiappi_template.php` originally reused the archive's `page-header--templates` key; it now has its own `page-header--template-single` class and Customizer entry ("Website Design — Single Design Page"), configured completely independently of `/templates/` — its SVG fallback still reuses `page-bg-templates.svg` since no dedicated illustration was commissioned for it, but its image/colour/animation choices in Customizer do not affect, and are not affected by, the archive's.
...
- **Per-plan "Includes Free Hosting"** (`_appiappi_plan_includes_free_hosting`, Pricing Plans meta box) and **"Website Design Credit"** (`_appiappi_plan_design_credit`) are both admin-set per plan. The whole pay-now/pay-later feature hinges on the first one: **the enforcement lives in exactly one place**, `appiappi_checkout_compute_order()` (`includes/pricing.php`) — a plan without free hosting always forces `payment_timing = 'now'` server-side regardless of what the client requested, and hosting becomes mandatory (selected + paid today) whenever the free-hosting perk isn't actually in effect: either the plan never has it, or its fee is being deferred (the perk only activates once the plan itself is actually paid). Paying a free-hosting plan in full today needs no hosting purchase at all. The Website Design credit only applies when paying in full today — never when deferring — and only ever reduces the design line (never below $0; any leftover past the design's price is forfeited, never applied to the plan or hosting cost).
