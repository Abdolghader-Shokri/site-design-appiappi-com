...
- `appiappi_template` CPT — registered in `wp-content/plugins/appiappi-template-showcase/includes/cpt.php`. Not public, uses the native Featured Image as the design preview (`post-thumbnails` support). Meta keys (prefixed `_appiappi_template_`): `desc`, `price`, `price_value` (a real float, added 2026-09-06, kept in sync with `price` for numeric sort/range-filtering — see §13), `rating`, `rating_count`, `demo_url`, `details_url`, `vendor`, `source_url` (the last two exist specifically so third-party designs credit their real source, per [MASTER_PROMPT.md § Website Template Library](MASTER_PROMPT.md#website-template-library)), `gallery` (added 2026-09-06 — newline-separated extra image URLs beyond the Featured Image, powers the card's multi-image carousel). One taxonomy: `appiappi_template_category` (hierarchical — a child term is a sub-category, shown in the card's subtitle; each term has an `icon` term-meta field set via custom add/edit-term fields in `includes/taxonomy-meta.php`, from the same curated icon set as the theme). The flat `appiappi_template_style` taxonomy that used to also exist here was removed 2026-09-06 — no longer a filterable dimension, per explicit request.
...
| **Page Header Backgrounds** (added 2026-09-06, extended same day) | Per page (Services, How It Works, Portfolio, Pricing, About, Contact, Website Designs, **and the Website Design single-design page as its own separate entry**): an optional background image, an optional background colour, and an "Animated Geometric Overlay" checkbox — all three independent per page. Image/colour override that page's default isometric SVG / `--color-bg-subtle`; the overlay draws `assets/js/page-header-network.js`'s colour-cycling node network on top. See §9 |
...
**Page-header decorative backgrounds (added 2026-09-06, revised/extended three times since):** `appiappi_page_header( $subtitle = '', $bg_key = '' )` (`inc/template-tags.php`) takes an optional second argument that adds a `.page-header--{key}` modifier class. Seven hand-authored isometric SVGs still exist in the repo (`assets/images/page-bg-{key}.svg` — Services, How It Works, Portfolio, Pricing, About, Contact, Templates) but are **no longer referenced as a default** (changed 2026-09-07, per explicit request): each `--page-header-bg-{key}` custom property now falls back to `none`, so a page header shows **no background image at all** until an admin explicitly uploads one for that page — there is no illustration to "replace" anymore. When an image *is* set, `background-size: 100% auto;` (changed 2026-09-07 from `cover`) scales it to the header's exact width with height following proportionally (never cropped, may letterbox vertically), `background-position: center`.

**Admin-overridable per page, three independent layers** — Customizer → **Page Header Backgrounds** gives each of the 8 pages (the 7 above, plus the Website Design **single-design** page as its own entry — see next paragraph) three controls:
- a `WP_Customize_Image_Control` upload → `--page-header-bg-{key}`, falling back to `none` (no default illustration, since 2026-09-07);
- a `WP_Customize_Color_Control` → `--page-header-bg-color-{key}`, falling back to `--color-bg-subtle` (background-image and background-color are separate CSS sub-properties, so a page can carry either, both, or neither independently);
- a checkbox, "Animated Geometric Overlay" → gates whether `appiappi_page_header_network_canvas( $bg_key )` prints a `<canvas class="page-header__network">` into that page's header at all (`appiappi_page_header_is_animated()`, both in `inc/template-tags.php`).

All three only emit a CSS var / DOM element when the admin actually set something — `appiappi_customizer_css_vars()` skips empty fields, and the canvas markup is entirely absent from pages where the toggle is off, so unused pages carry zero extra DOM/JS cost. A `:root`-level custom property inherits down regardless of the consuming selector's specificity, so none of this needed `!important` or cascade trickery.

**Animated geometric overlay** (`assets/js/page-header-network.js`, appended to `main.js`): when its canvas is present, a small IIFE draws a scatter of nodes (scaled to the header's area) that drift slowly, bounce off the header's edges, and connect with thin lines to nearby nodes; both nodes and lines cycle through a shifting multi-hue HSL palette (each node has its own hue + drift speed, so the mix is genuinely varied, not a single tinted colour) with a soft glow (`shadowBlur`) on each node — built after the user supplied a reference image of a white plexus/network graphic and asked for the same *shape* language but colourful and combined/random rather than monochrome-white, layered on top of whichever image/colour that page already has (the overlay itself has a fully transparent canvas background). Draws one static frame and skips the animation loop under `prefers-reduced-motion`, and pauses via `visibilitychange` while the tab is hidden. No external library — vanilla `<canvas>`, consistent with the rest of the theme's JS.

**Overlay speed/density/line-glow are global, not per page (added 2026-09-07):** unlike the per-page image/colour/on-off toggle above, three more controls in the same Customizer section apply identically to every page's overlay at once, per explicit request ("لازم نیست این سه درخواست برای هر برگه جدا باشه") — Overlay Dot Speed (`appiappi_pagebg_network_speed`, 0.25–3×, default 1, multiplies each node's drift velocity), Overlay Dot Density (`appiappi_pagebg_network_density`, 0.3–2.5×, default 1, multiplies the node count formula, clamped to 8–140 nodes), and Animate Connection Lines (`appiappi_pagebg_network_glow_lines`, checkbox, default off). All three are read once in `appiappi_enqueue_assets()` (`inc/enqueue.php`) via `get_theme_mod()` and handed to `main.js` as `window.appiappiPageHeaderNetwork` (`wp_localize_script`) — cheap to always print since the overlay script itself no-ops on any page with no canvas. When "Animate Connection Lines" is on, each line's width/opacity/glow pulses via a sine wave seeded by that line's length (`draw( timestamp )` now receives the `requestAnimationFrame` timestamp) so lines shimmer independently rather than in lockstep — the "floating in space" look the user asked for; off (default) keeps the original flat, static-opacity lines. Values are clamped server-side (`appiappi_sanitize_decimal_multiplier`, [0.1, 5]) so a tampered value can't send the node count or velocity somewhere absurd.

**Performance pass, same day — this overlay runs on all 8 page headers, so its CPU cost is a real (if indirect) SEO consideration via mobile Core Web Vitals/INP, not just a UX nicety.** Three changes, no visual regression: (1) both the per-node glow and the new glow-lines effect switched from `ctx.shadowBlur` — a comparatively expensive canvas operation — to a cheaper "halo" technique (a second, larger, low-opacity fill/stroke drawn under the bright core), which looks near-identical but avoids the extra compositing pass `shadowBlur` forces on every shape, every frame; (2) the redraw loop is capped at 30fps (`frameInterval`) instead of an uncapped ~60fps+, since slow-drifting dots don't read as choppier at half the rate, roughly halving the cost of the O(n²) distance check that dominates this file; (3) an `IntersectionObserver` now pauses the animation once its header scrolls out of the viewport, on top of the existing tab-hidden pause — previously the loop kept running (and contending for the main thread) for the entire time a tab stayed open, even long after a visitor scrolled past the header. Degrades safely on browsers without `IntersectionObserver` (the animation simply never pauses for that reason, same as before this change).

**Trust bar was the fifth occurrence of the recurring "shared markup styled in a front-page-only CSS file" mistake (fixed 2026-09-07):** `template-parts/sections/trust-bar.php` (the 4-icon strip) is used by both `front-page.php` and `page-about.php`, but `.trust-bar`/`.trust-bar__grid` — the rules that actually lay the 4 items out in a row — lived only in `home.css`, which `inc/enqueue.php` only enqueues on `is_front_page()`. The About page therefore got the icons with zero layout CSS, rendering them stacked on top of each other. Moved both rules into `components.css` (§10, loaded on every page) instead of duplicating them into `pages.css`; `.icon-tile` itself was already there from an earlier fix, so this closes the gap for good rather than patching it per page again.

**About page — its own desktop side padding + a header subtitle for consistency (added 2026-09-07):** the sitewide 10px `--container-pad` read as too tight for this much long-form editorial copy — `page-about.php`'s content wrapper gained an `.about-page-content` class with its own `padding-inline: 50px` at ≥768px (`pages.css`), the same "dedicated override class" pattern as `.single-template-detail`'s 30px. It also previously passed an empty subtitle to `appiappi_page_header()` — every other main page passes one — so it now gets a real one-line subtitle matching the rest of the site's tone.

**Page title colour, stroke and background box — global, added 2026-09-07:** a fourth Customizer section, **Page Title Styling**, styles the H1 (and subtitle, where one exists) inside every one of the same 8 page headers — one shared style, not set per page, same reasoning as the overlay speed/density controls above. Three independent, all-optional layers: a **Title Text Colour** (`appiappi_pagetitle_color`, falls back to the theme's existing `--color-neutral-900` heading colour when unset); a **stroke/outline** (`appiappi_pagetitle_stroke_enabled` checkbox + `appiappi_pagetitle_stroke_color` + `appiappi_pagetitle_stroke_width` range, 0.5–5px) rendered via `-webkit-text-stroke-width`/`-color` — the enabled flag is its own boolean, kept separate from the saved colour/width, so switching it off doesn't discard those for later; and a **background box** (`appiappi_pagetitle_bg_color` + `appiappi_pagetitle_bg_opacity` range, 0–100%) — an inline-block, rounded, shadowed badge behind the title text (`.page-header__title-box`, `border-radius: var(--radius-lg); box-shadow: var(--shadow-lg)`), whose opacity is layered on top of the chosen colour via `color-mix()` so the two controls stay independent. The box follows the same "no default, opt in" pattern as the page header background images: `appiappi_page_title_bg_is_enabled()` (`inc/template-tags.php`) gates whether the wrapper markup is printed at all, purely on whether a colour has been chosen — no colour means no box and no shadow, not an invisible one. `appiappi_page_header()` and the single design template's hand-built header both call the same two shared helpers (`appiappi_page_title_box_open()`/`_close()`) and add the `.page-header--title-stroke` class themselves when the stroke toggle is on, so both stay in sync automatically. Colour and stroke initially only styled the `<h1>` in `pages.css`; extended the same day to also cover `.page-header p` (the subtitle passed as `appiappi_page_header()`'s first argument) once the user pointed out it should get identical treatment — the subtitle's own `var()` fallback (`--color-text-muted`) is kept separate from the H1's (`--color-neutral-900`) so nothing changes until a colour is actually set, at which point both adopt it together.

**Single-design page — its own background, separate from the archive (revised 2026-09-06):** `single-appiappi_template.php` originally reused the archive's `page-header--templates` key; it now has its own `page-header--template-single` class and Customizer entry ("Website Design — Single Design Page"), configured completely independently of `/templates/` — its image/colour/animation choices in Customizer do not affect, and are not affected by, the archive's. Like every other page-header key, it now falls back to no image at all until one is uploaded for it specifically (see above).
...
**Design card overhaul (2026-09-06):** a large pass on the card/grid UI, `appiappi_render_template_showcase()`'s signature, and the data shape (`appiappi_showcase_map_post()`), all per explicit request:
...
- **Category/sub-category subtitle:** small, muted, italic line under the card title (`.template-card__group`) showing the design's category and, if its assigned term is itself a child of another (the taxonomy has always been hierarchical, just unused for this before now), " › {child name}" too. No new field — `appiappi_showcase_map_post()` just walks `$category->parent` and looks up the parent term. `appiappi_showcase_get_categories()` now passes `'parent' => 0` so the sidebar's filter list only shows top-level categories (a design assigned to a child term still matches its parent's filter link — `tax_query`'s `include_children` defaults to true).
- **Real, server-side price range filter + sort — deliberately not client-side JS**, unlike the search box: at a catalogue that may reach ~2,000 designs, "sort by price" or "under $30" has to work correctly across the whole, correctly-paginated result set, not just whichever 12 happen to already be on the page. New `_appiappi_template_price_value` meta (a real float, kept in sync with the display `_appiappi_template_price` string by both the meta box's save handler and `price-sync.php` — `appiappi_showcase_parse_price_value()` in `cpt.php` is the one shared parser) makes `orderby=meta_value_num` sorting and `meta_query` range filtering actually work (a bare `"$59"` string can't be cast to a number reliably). `appiappi_showcase_get_price_range()` computes the sitewide min/max via one SQL aggregate (not a PHP loop over every design), cached in a transient invalidated on every design save — powers the sidebar's Min/Max filter inputs and their placeholder text. `appiappi_showcase_archive_query()` reads `?min_price=`/`?max_price=` (a `meta_query` clause) and one combined `?sort=price-asc|price-desc|rating-desc` (rating didn't need a new field — it was already stored as a plain, sortable decimal string). The price filter and sort controls only render on the real `/templates/` archive (gated on `is_post_type_archive()`/a non-null `$price_range`), never the homepage teaser, since only the archive's query actually reads those parameters — showing them on the teaser would be a non-functional dead UI element.
- **Content seeded 2026-09-06:** backfilled `_appiappi_template_price_value` for all 13 existing designs (parsed from their existing display price), and re-fetched the Real Estate search results to backfill `_appiappi_template_gallery` for the 2 of the original 10 (`Landor`, `TheNessVilla`) that actually have more than one source image available — the other 8 genuinely only have one, confirmed against the same Envato search response, not a gap in the original seeding.
...
## 22. Payment System (Checkout) — Built 2026-09-06

**Built** — a new companion plugin, `appiappi-checkout`, added a real Stripe-backed checkout flow on top of the existing Pricing page. Customer Portal and a Support/Request Management system are still **not started** — requirements for those two remain in [MASTER_PROMPT.md § Customer Portal](MASTER_PROMPT.md#customer-portal--future-ready) and [§ Support / Request Management](MASTER_PROMPT.md#support--request-management--future-system). The "Client Login" header link still just reserves `/account/` as a URL target for whenever those get built.

**The flow:** "Choose This Design" (grid card or the single design page) now links to `/pricing/?design_id={post ID}` instead of the Contact page. The Pricing page looks that ID up server-side (`appiappi_checkout_get_selected_design()` — the URL is only ever a lookup key, the price shown is always the design's real, current price) and adds a "Website Design: {name} — +$X" line to every plan card. Each plan's own button (`appiappi_render_pricing_cards()` in the theme, gated on `appiappi_checkout_is_configured()` existing) opens a themed checkout modal (`--checkout-plan-color` set from the same `--plan-color` the card itself uses) instead of navigating anywhere: an invoice (plan, the design line if present, a Monthly/Annual toggle for plans whose `_appiappi_plan_billing_frequency` is `monthly`, with a configurable annual discount — default 5%, Settings → Appiappi Checkout), a short contact form, then Stripe's Payment Element for the actual card/wallet details. Payment methods shown there are whatever's enabled in the Stripe Dashboard (`automatic_payment_methods`) — no code here decides that.

**Server-side pricing** (`includes/pricing.php`) is the one place amounts get computed, from a plan ID and a design post ID looked up fresh against their own CPTs — never from anything the browser sends. A `one_time` plan gets a single Stripe PaymentIntent (design price folded straight into the amount); a `monthly`/`yearly` plan gets a real Stripe Subscription built with inline `price_data` (no pre-created Stripe Products needed) plus the design price as a one-off `add_invoice_items` entry on the *first* invoice only — the design is a one-time build cost, not something that should recur every billing cycle.

**Orders** (`appiappi_order` CPT, private/admin-only, same pattern as `appiappi_lead`) are created as `pending` the moment a PaymentIntent/Subscription is created, then flipped to `paid`/`failed` exclusively by the Stripe webhook (`includes/webhook.php`, route `appiappi/v1/stripe-webhook`, manual HMAC-SHA256 signature verification per Stripe's documented algorithm since this plugin doesn't use the Stripe SDK) — never trusted from the browser alone, since a closed tab or a 3-D Secure redirect mid-flow means the frontend can't reliably know whether payment actually succeeded. A short-lived AJAX poll (`appiappi_checkout_order_status`) gives the visitor a real-time "paid"/"failed" confirmation in the same modal without waiting on email.

**Hosting, pay-now/pay-later, and design credit — added 2026-09-07.** Three more moving pieces layered onto the checkout above:

- **Hosting Packages** (`appiappi_hosting` CPT, under Orders → Hosting Packages, native create/edit/delete, no custom admin UI needed): each is one Location + Storage (specific amount or Unlimited) + Traffic (specific amount or Unlimited) + annual price combination. At checkout, a customer narrows down to one package through cascading Location → Storage → Traffic selects (`checkout.js`); the Traffic step is skipped entirely when every package matching the chosen Location + Storage already shares the same traffic tier.
- **Per-plan "Includes Free Hosting"** (`_appiappi_plan_includes_free_hosting`, Pricing Plans meta box) and **"Website Design Credit"** (`_appiappi_plan_design_credit`) are both admin-set per plan. The whole pay-now/pay-later feature hinges on the first one: **the enforcement lives in exactly one place**, `appiappi_checkout_compute_order()` (`includes/pricing.php`) — a plan without free hosting always forces `payment_timing = 'now'` server-side regardless of what the client requested. **A hosting package selection is always required, in every scenario** (revised 2026-09-07: previously it was skipped entirely for a free-hosting-plan-paying-now, but the business still needs to know which package to provision even when it costs the customer nothing) — what varies is only whether it's *charged*: `hosting_is_free` is true (hosting_price = 0, but `hosting_original_price` is still returned) only when the plan includes free hosting AND the customer is paying in full today; every other combination charges the real annual price. The checkout modal always shows the cascading hosting selector and always shows a hosting invoice line — when free, it displays the real price struck through next to a "FREE" badge and a short note explaining why, rather than hiding the line. The Website Design credit only applies when paying in full today — never when deferring — and only ever reduces the design line (never below $0; any leftover past the design's price is forfeited, never applied to the plan or hosting cost).
- **Deferred plan fees are a manual follow-up, not automated.** When `payment_timing = 'later'`, only design (after credit) + hosting are charged today via a single PaymentIntent; no Stripe Subscription/PaymentIntent for the plan's own fee is created at all. The Order record (`_appiappi_order_deferred_amount`, `_appiappi_order_payment_timing`) tells the business owner what's still owed and when to collect it — "work completed" isn't a fixed calendar date Stripe could bill against on its own, so automating that collection was deliberately left out of this version.

**Currency is fixed at USD** (`appiappi_checkout_currency()`, `inc/settings.php`) — caught and fixed the same day: Website Design prices come from Envato/ThemeForest, which always lists in USD regardless of buyer location, and the checkout math was adding those straight to the (CAD-labelled) plan prices as if they were the same unit. Rather than run a live exchange-rate conversion or make the buyer pick a currency mid-checkout, the user explicitly chose to charge everything in USD and let a Canadian cardholder's own bank/card network convert at settlement, same as any other US-priced purchase — the standard, lowest-complexity approach. This is deliberately decoupled from the theme's general "Currency Symbol/Code" setting (Settings → Appiappi Settings, still CAD, used for on-page display elsewhere), not reused from it.

**Going live is deliberately just pasting keys in**: Settings → Appiappi Checkout has Stripe Publishable Key, Secret Key, and Webhook Signing Secret fields, plus the annual discount %. Test vs. live mode is read from the secret key's own prefix (`sk_test_`/`sk_live_`) rather than a separate toggle — nothing else in the plugin should ever need touching after a host move, aside from the one required manual step on Stripe's own side: registering the webhook URL (shown on the settings page, built from `rest_url()` so it's correct on any domain automatically) in the Stripe Dashboard and pasting back the signing secret it gives you.

## 22a. Client Login (Google Sign-In) — Built 2026-09-07

New companion plugin `appiappi-client-login`: customers sign in with their Google account only — there is no separate registration form anywhere. Signing in successfully with Google for the first time *is* the account-creation step. Built as plain REST calls via `wp_remote_request()` (no SDK, no Composer), matching the Stripe integration's style exactly.

**OAuth flow** (`includes/oauth.php`): `appiappi_client_login_render_signin_button()` links to Google's consent screen (`accounts.google.com/o/oauth2/v2/auth`, scope `openid email profile`) with a random single-use `state` value stored server-side in a transient (never trusted from the browser) alongside the page to return to. Google redirects back to `appiappi_client_login_redirect_uri()` — a REST route, `rest_url('appiappi/v1/google-callback')`, correct on any domain automatically — which exchanges the one-time `code` for an access token (`oauth2.googleapis.com/token`), calls Google's own `userinfo` endpoint with that token to get an authoritative email (never a JWT we'd have to verify ourselves), finds or creates the matching WP user, logs them in (`wp_set_auth_cookie()`), and redirects back to wherever sign-in started. Any failure along the way redirects back with `?client_login_error=1`, which prints a small dismissible banner (`wp_body_open`) — `client-login.js` also strips the query arg from the address bar once shown.

**Account provisioning** (`includes/user.php`): a dedicated **Client** role (`appiappi_client`, capability: `read` only — nothing else) is registered on activation and assigned to every auto-created user; `appiappi_client_login_find_or_create_user()` looks up by email first (idempotent — signing in again never creates a duplicate), and only creates a new `wp_insert_user()` on a genuine first sign-in, with a random unused password that is never actually used to log in (Google is the only sign-in path for this role). Client accounts are redirected away from `/wp-admin/` entirely (`admin_init` hook) straight to their Dashboard page, since `wp-admin` would just be a confusing, irrelevant screen for a `read`-only account.

**Dashboard/Profile pages** (`includes/pages.php`): two real WP pages are created once on activation — Dashboard (slug `account`, matching the header's pre-existing `/account/` link) and Profile (a child of Dashboard, so its URL is `/account/profile/` via WP's normal parent/child permalink nesting) — each served by the plugin's own template via `template_include` (`templates/page-dashboard.php` / `page-profile.php`). Both show a "Sign in with Google" prompt when logged out, or a minimal placeholder (welcome message + email) when logged in — deliberately minimal for now: this phase is the sign-in mechanism and header navigation, not the dashboard/profile features themselves (project status, invoices, editable profile fields), which are an explicitly deferred future phase.

**Header integration** (`includes/header-widget.php` + theme's `template-parts/header/site-header.php`): the theme calls `appiappi_client_login_render_header_widget( $classes )` through a `function_exists()` guard — the exact same decoupling pattern the checkout plugin uses for the pricing cards — falling back to a plain static "Client Login" link if this plugin is ever deactivated. Logged out, it renders the Google-branded sign-in button (Google's own multi-colour "G" mark inlined as SVG, no image request); logged in, it renders an email + generic person-icon avatar with a **click-to-toggle** dropdown (not hover-only, so it behaves identically on touch — `client-login.js` closes it on an outside click, a second trigger click, or Escape) containing Profile, Dashboard and Log Out. Log Out uses core's own `wp_logout_url( home_url('/') )` — no custom code needed, WP handles the nonce-protected logout + redirect itself. On the mobile slide-out nav panel, the same widget expands inline (`position: static`) instead of floating, since a full-width tap target reads better in a narrow panel than a small floating dropdown.

**Going live is, again, deliberately just pasting keys in**: Settings → Client Login has Google Client ID and Client Secret fields plus a read-only Redirect URI field to copy into Google Cloud Console. The one unavoidable manual step — Google requires a domain-specific "Authorized redirect URI" registered in advance, so this can't be fully automatic — mirrors the Stripe webhook registration step exactly.

**Not yet built (explicitly deferred, per the user)**: any real Dashboard/Profile content — this conversation's request was scoped to the sign-in mechanism and header menu only, with the rest to be specified in a later request.
